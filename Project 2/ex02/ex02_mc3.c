...
// ΠΡΟΣΘΗΚΗ Mutex για ουρά task
pthread_mutex_t queue_mutex = PTHREAD_MUTEX_INITIALIZER;
int next_task_index = 0; // Δείκτης του επόμενου task προς εκτέλεση
...
void * outer_worker(void *args) {
    ...
    /* int P = my_args->total_outer_threads; //αριθμος εξωτερικων νηματων (Δεν χρειαζεται για το bonus) */ 
    int T = my_args->inner_threads_T;     //αριθμος εσωτερικων νηματων
    unsigned long long n = my_args->n;    //αριθμος δειγματων
    
    // ΠΡΟΣΘΗΚΗ Loop μέχρι να αδειάσει η ουρά task
    while (1) {
        int k = -1; // Το index του task που θα αναλάβει το νήμα

        // Κλείδωμα ουράς για ανάληψη task
        pthread_mutex_lock(&queue_mutex);

        if (next_task_index < my_args->ntasks) {
            // Ανάθεση task index στο νήμα
            k = next_task_index;
            next_task_index++;
        }
        // Ξελείδωμα ουράς για ανάληψη task από επόμενο νήμα
        pthread_mutex_unlock(&queue_mutex);

        // Αν δεν υπάρχει άλλο task (k == -1), τερματίζουμε το νήμα
        if (k == -1) {
            break;
        }
...
int main(int argc, char *argv[]) {
  ...
  printf("Two-Level Parallel Monte Carlo with Task Queue (n=%llu samples per task, P=%d Outer threads, T=%d Inner threads per task)\n",n, P, T);
