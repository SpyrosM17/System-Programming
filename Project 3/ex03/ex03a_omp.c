...
//Τροποποιηση (+ task_id και num_threads_inner στις παραμέτρους)
static double integrate_parallel_mc(double (*f)(double), double a, double b, unsigned long long n, int task_id, int num_threads_inner) {
  ...
  // Εσωτερικός Παραλληλσιμός, reduce global_sum στο για ασφαλή άθροιση των μερικών αποτελεσμάτων
  #pragma omp parallel num_threads(num_threads_inner) reduction(+:global_sum)
  {
    int tid = omp_get_thread_num(); //  ID του thread
    
    // Αντικατάσταση της srand48/drand48 με erand48 που δέχεται local state (xsubi).
    ...
    // Διαμοιρασμός του loop (work-sharing) στα νήματα.
    #pragma omp for schedule(static) nowait //Αποφυγή barrier
    for (i = 0; i < n; ++i) {
      double r = erand48(xsubi); // Τροποποιηση, Thread-safe RNG με local-state
      double xi = a + width * r;
      local_sum += f(xi);
    }
...
int main(int argc, char *argv[]) {
  unsigned long long n = 200000000ULL; 
  ...
  //Ενεργοποίηση nested parallelism για να επιτραπεί η δημιουργία νημάτων μέσα σε νήματα
  omp_set_nested(1);
